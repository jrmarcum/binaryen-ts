...
| [passes.md](passes.md)             | The optimization pass set + pass-runner; per-pass design (Vacuum, OptimizeInstructions, CoalesceLocals + CFG liveness, LocalCSE, Inlining + split/partial + return-call, RemoveUnused*, PickLoadSigns); **Asyncify port IN PROGRESS (Stage 2/5 — runtime support + analyzeModule instrument-set, oracle-validated vs wasm-opt v130; Stage 3 needs a flatten pass first; for TinyGo goroutines; resume point inside)** |
...
| [testing.md](testing.md)           | How to run `deno task check` / `test` / `ci`; the Deno-only test suite (358 passing, 1 ignored as of 2026-07-05; +7 S1 +10 S2 asyncify); the seeded differential optimizer fuzzer (`optimize_fuzz_test.ts`) and why behavioral fuzzing exists                                                                                                                  |
