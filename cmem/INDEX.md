...
| [passes.md](passes.md)             | The optimization pass set + pass-runner; per-pass design (Vacuum, OptimizeInstructions, CoalesceLocals + CFG liveness, LocalCSE, Inlining + split/partial + return-call, RemoveUnused*, PickLoadSigns); **Asyncify port IN PROGRESS (Stage 1/5 — runtime support, oracle-validated vs wasm-opt v130; for TinyGo goroutines; resume point inside)** |
...
| [testing.md](testing.md)           | How to run `deno task check` / `test` / `ci`; the Deno-only test suite (348 passing, 1 ignored as of 2026-07-05; +7 asyncify Stage 1); the seeded differential optimizer fuzzer (`optimize_fuzz_test.ts`) and why behavioral fuzzing exists                                                                                                                  |
